class RecordsController < ApplicationController
  ...
  def update
    ...
    #old notification process - detective work, The new one is done mainly out of messgs controller. its more directed so its much simpler.
...
#      Notification.createnotification(current_user.id,recescrow, @record.id, mychange, oldchange, newchange)
  ...
  def sendmsg()
#  params[:themessage, :sendadmin, :sendmaster, :sendborrower, :sendcoborrower, :sendprocessor, :sendrealtor, :sendloanofficer, :sendescrowofficer, :sendmarketer].blank
#  when you hit submit it goes to the records controller in a specific def that looks at all the parameters of the check boxes. if :realtor = true then
#  sendtoteam = sendtoteam + "; " + record.realtor.email
#  ...
#  if barrower = true then
#  sendtocustomer = sendtocustomer +"; "+ record.borrower.email
#  introteam = "this was sent to "+record.borrower.name+":"
#  introcustomer = "Dear "+record.borrower.name
#  ...
#  else
#  introteam = "progress was made on "+record.borrower.name +" loan:"
#  introcustomer = "None"
#  endif
#
#  sendmsg(introteam, messg.messg, sendtoteam)
#  if introcustomer != "None
#  sendmsg(introcustomer, messg.messg, sendtobarrower)
  ...
  def addstep
    ...
    #create step
    Step.where(record_id: params[:id], progression_id: params[:progression_id], profile_id: params[:profile_id]).first_or_create
    ...
    if myprogression.messg.message.blank?
      ...
      #redirect_to messg_path(myprogression.messg)
      redirect_to :controller => 'messgs', :action => 'show', :id => myprogression.messg.id, :recordid => myrecord.id
    end
  end
  ...
  private
    ...
    def record_params
      params.require(:record).permit( :firstname,
                                      :lastname,
                                      :phone,
                                      :email,
                                      :raemail,
